...
int main(int argc, char ** argv) {
    ...
    // Add cloud
    execute_txn([myName] () {
        if (cloudList.Index(myName) == -1) cloudList.Append(myName);
    },
    ...
    reactive_txn([myName] () {
		...
		if (index != -1)
		if (deviceStatusList.Value(index) != -1)
    ...
    // Cycle on user input to add device
    while (1) {
	string deviceName;
	cin >> deviceName;
        // Add device
        execute_txn([myName,deviceName] () {
            if (deviceList.Index(deviceName) == -1){
		deviceList.Insert(uniqueId.Value(),deviceName);
		deviceStatusList.Insert(uniqueId.Value(),2);
	    }
	    ++uniqueId;
        },
        // txn callback: If transaction fails, exit
